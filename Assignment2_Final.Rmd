...
In this second part, we tried different preparation to see whether it would have better F1 score.
...
The result is better in this model (0.5736)
...
```{r}
actual_class <- test$suggestion
predicted_class <- predict(fit.knn, newdata = test)
tab_class <- table(actual_class, predicted_class)
tab_class
...
We tried two different data prep approaches to try slightly different things and to see what could work better. However the performance for both was rather underwhelming. Since we also used other algorithms on Python which performed significantly better and actually beat naive bayes, glmnet, random forrest, knn and xgboost and other models by a very large margin, we ended up not doing as much comparing and tuning them as planned (we did some tuning and tried differnt algos etc.) because we did not feel like we could come close to the Python algos. 

If we were to further try it with we would try the following things: 
1. tune more here, make a larger grid run tuning for longer. 
2. Try it with 2-grams more. We tried 2-grams with naive bayes and Glmnet but it didn't yield better results, so went back to 1-grams. We could also even try it with 3-grams etc. 
3. we could use other dimensionality reducing techniques or refrain form using the reduced dataset for glmnet or xgboost to try and see if that could get better results. 
